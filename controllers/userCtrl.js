    ...
    try {
      ...
      if (password === name || password === email)
        return res
          .status(400)
          .json({
            msg: 'Your password must be different than your name or email.',
          });
    ...
    try {
      ...
      if (existintgEmail === newEmail) {
        return res
          .status(400)
          .json({
            msg: 'This email is already taken, try with something else',
          });
      }
  ...
  deleteAccount: async (req, res) => {
    try {
      ...
      if (query.deletedCount === 0)
        return res
          .status(500)
          .json({ msg: "This user couldn't be delete, please try again" });
